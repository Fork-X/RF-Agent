...
```bash
python3 src/quangan/skills/ncm-cli-setup/scripts/install_mpv.py
```

**Linux**：询问用户是否需要播放功能：

- 如果是桌面环境、需要本地播放 → 执行 `python3 src/quangan/skills/ncm-cli-setup/scripts/install_mpv.py`
...
| `mpv not found`              | 重新运行 `python3 src/quangan/skills/ncm-cli-setup/scripts/install_mpv.py` 或手动安装：https://mpv.io |
